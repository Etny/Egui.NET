...
const EGUI_FNS: EguiFnMap = AUTOGENERATED_EGUI_FNS
   ...
   .with(EguiFn::egui_containers_collapsing_header_CollapsingState_show_toggle_button, |mut state: CollapsingState, mut ui: EguiPointer<Ui>, callback: EguiCallback| unsafe {
    ...
    // Bindings for Slider
    .with(EguiFn::egui_widgets_slider_Slider_ui, |mut ui: EguiPointer<Ui>, obj: SerializableSlider, mut value: f64, formatter: Option<EguiCallbackr<*const c_char>>| unsafe {
...
// }
//
pub type EguiCallback = EguiCallbackr<()>;
/// A C# callback for `egui` to invoke.
#[derive(Copy, Clone)]
#[repr(C)]
pub struct EguiCallbackr<T> {
    /// The function to call.
...
unsafe impl<T> Send for EguiCallbackr<T> {}
unsafe impl<T> Sync for EguiCallbackr<T> {}

impl<T> EguiCallbackr<T> {
    /// Invokes the callback.
    /// 
    /// # Safety
    /// 
...
impl<T> Serialize for EguiCallbackr<T> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer {
        [self.func as usize, self.data as usize].serialize(serializer)
...
impl<'a, T> Deserialize<'a> for EguiCallbackr<T> {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
        where
            D: Deserializer<'a> {
        unsafe {
...
impl SerializableSlider {
    ...
    pub fn to_slider<'a>(self, value: &'a mut f64, formatter: Option<EguiCallbackr<*const c_char>>) -> Slider<'a> {
