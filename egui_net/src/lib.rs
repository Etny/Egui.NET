...
const EGUI_FNS: EguiFnMap = AUTOGENERATED_EGUI_FNS
   ...
   .with(EguiFn::egui_containers_collapsing_header_CollapsingState_show_toggle_button, |mut state: CollapsingState, mut ui: EguiPointer<Ui>, callback: EguiCallback| unsafe {
    ...
    // Bindings for Slider
    .with(EguiFn::egui_widgets_slider_Slider_ui, |mut ui: EguiPointer<Ui>, obj: SerializableSlider, mut value: f64, formatter: Option<EguiCallbackFn<*const c_char>>| unsafe {
        (ui.get_mut().add(obj.to_slider(&mut value, formatter)), value)
    })
...
// #[derive(Copy, Clone)]
// #[repr(C)]
// pub struct EguiCallbackStr {
//     /// The function to call.
//     func: unsafe extern "C" fn(*const c_void, *const c_void) -> *const c_char,
//     /// Data to pass as the second function argument.
//     data: *const c_void
// }
// unsafe impl Send for EguiCallbackStr {}
// unsafe impl Sync for EguiCallbackStr {}
//
// impl EguiCallbackStr {
//     /// Invokes the callback.
//     /// 
//     /// # Safety
//     /// 
//     /// All invariants for [`Self::func`] must be satisfied.
//     pub unsafe fn invoke(&self, argument: *const c_void) -> *const c_char {
//         (self.func)(argument, self.data)
//     }
// }
//
// impl Serialize for EguiCallbackStr {
...
// }
//
// impl<'a> Deserialize<'a> for EguiCallbackStr {
...
// }
//
pub type EguiCallback = EguiCallbackFn<()>;
/// A C# callback for `egui` to invoke.
#[derive(Copy, Clone)]
#[repr(C)]
pub struct EguiCallbackFn<T> {
    /// The function to call.
    func: unsafe extern "C" fn(*const c_void, *const c_void) -> T,
    /// Data to pass as the second function argument.
    data: *const c_void
}

unsafe impl<T> Send for EguiCallbackFn<T> {}
unsafe impl<T> Sync for EguiCallbackFn<T> {}

impl<T> EguiCallbackFn<T> {
    /// Invokes the callback.
    /// 
    /// # Safety
    /// 
    /// All invariants for [`Self::func`] must be satisfied.
    pub unsafe fn invoke(&self, argument: *const c_void) -> T {
        (self.func)(argument, self.data)
    }
}

impl<T> Serialize for EguiCallbackFn<T> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer {
        [self.func as usize, self.data as usize].serialize(serializer)
...
impl<'a, T> Deserialize<'a> for EguiCallbackFn<T> {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
        where
            D: Deserializer<'a> {
        unsafe {
...
impl SerializableSlider {
    ...
    pub fn to_slider<'a>(self, value: &'a mut f64, formatter: Option<EguiCallbackFn<*const c_char>>) -> Slider<'a> {
        ...
        result = result.custom_formatter(move |n: f64, _| { 
            let c_str = unsafe { std::ffi::CStr::from_ptr(formatter.invoke(&n as *const _ as *const _))};
